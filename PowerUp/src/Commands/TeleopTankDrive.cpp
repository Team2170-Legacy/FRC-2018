...
TeleopTankDrive::TeleopTankDrive() :
		frc::Command(), driveMode(
				(DriveMode) Preferences::GetInstance()->GetInt("Drive Mode",
						tankDrive)), mThrottleTimeConstant(
				Preferences::GetInstance()->GetDouble("Throttle TC", 0.180)), driverXbox(
				0) {
	...
	ThrottleInput.reset(new FilterableDouble());
	ThrottleFilter.reset(
			new LinearDigitalFilter(
					LinearDigitalFilter::SinglePoleIIR(ThrottleInput, 0.02,
							mThrottleTimeConstant)));
}
